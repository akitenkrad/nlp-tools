...
class Config(object):
    ...
    def __load_config__(self, config_path:PathLike, ex_args:dict=None):
        ...
        self.__config__['log']['log_dir'] = Path(self.__config__['log']['log_dir']) / f"{self.__config__['train']['exp_name']}_{self.__config__['timestamp'].strftime('%Y%m%d%H%M%S')}"
...
def __show_progress__(block_count, block_size, total_size):
    ...
    print(Fore.LIGHTCYAN_EX, f'[{bar}] {percentage:.2f}% ( {total_size_kb:.0f}KB )\r', end='')
