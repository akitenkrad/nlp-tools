...
def evidence_extractor_loss(
    ...
    end_y: torch.Tensor,
    psg_ranks: torch.Tensor,
    psg_ranks_y: torch.Tensor,
):
...
class SNetEvidenceExtractor(BaseModel):
    ...
    def step(self, x: MsmarcoItemX, y: MsmarcoItemPos, loss_func: Callable) -> Tuple[Any, Any]:
        ...
        loss = loss_func(
            ...
            self.loss_r,
            a_1,
            y.start_pos.to(self.config.train.device),
            a_2,
            y.end_pos.to(self.config.train.device),
            psg_ranks,
            x.passage_is_selected.to(self.config.train.device),
        )
