...
class LivedoorNewsDataset(BaseDataset):
    def __init__(self, config: Config, embedding: Embedding, test_size: float = 0.1, valid_size: float = 0.1):
        ...
        self.train_data, self.valid_data, self.test_data = self.__load_data__(
            self.dataset_path, self.test_size, self.valid_size
        )

    def __load_data__(
        self, dataset_path: Path, test_size: float, valid_size: float
    ) -> Tuple[LivedoorDs, LivedoorDs, LivedoorDs]:
        ...
        files = [
            Path(f)
            for f in glob(str(dataset_path / "livedoor-news-corpus" / "**" / "*.txt"))
