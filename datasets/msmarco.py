...
class MsmarcoRecord(object):
    ...
    def to_data(self, embedding: Embedding) -> dict:
        ...
        data = {
            ...
            'query_word_tokens': torch.tensor(qwt, dtype=torch.float64),
            'query_char_tokens': [torch.tensor(chars, dtype=torch.float64) for chars in qct],
            'psg_word_tokens': [torch.tensor(passage, dtype=torch.float64) for passage in pwt],
            'psg_char_tokens': [[torch.tensor(chars, dtype=torch.float64) for chars in passage] for passage in pct],
            'psg_is_selected': torch.tensor(psl, dtype=torch.float64),
            'ans_word_tokens': [torch.tensor(answer, dtype=torch.float64) for answer in awt],
            'ans_char_tokens': [[torch.tensor(chars, dtype=torch.float64) for chars in answer] for answer in act],
